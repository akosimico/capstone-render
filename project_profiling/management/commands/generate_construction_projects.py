...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--with-tasks',
            action='store_true',
            help='Generate tasks and scopes for projects'
        )
        parser.add_argument(
            '--with-budgets',
            action='store_true',
            help='Generate budgets and allocations'
        )
        parser.add_argument(
            '--with-notifications',
            action='store_true',
        ...
        parser.add_argument(
            '--clear-existing',
            action='store_true',
            help='Clear existing data before generating new'
        )
    ...
    def handle(self, *args, **options):
        ...
        if options['with_budgets']:
            if ProjectBudget and FundAllocation and CostCategory:
                self.create_budgets_and_allocations(projects)
            else:
                self.stdout.write(self.style.WARNING('Budget models not found, skipping budget generation'))
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully generated {num_projects} construction projects!'
            )
        )
    ...
    def create_project_types(self):
        """Create realistic project types"""
        project_types = [
        ...
        for pt_data in project_types:
            ...
            if created:
                self.stdout.write(f'  Created project type: {pt.name}')
    ...
    def create_users(self):
        ...
        for role_name, role_code, count in roles_data:
            for i in range(count):
                # Create Django user (CustomUser uses email as primary field)
                ...
                if profile_created:
                    self.stdout.write(f'  Created {role_name}: {profile.full_name}')

    def create_clients(self):
        """Create sample clients"""
        # Philippine construction companies and clients
        clients_data = [
    ...
    def create_projects(self, count):
        """Create realistic construction projects"""
        projects = []
        ...
        for i in range(count):
            ...
            budget_ranges = {
                'RES': (500000, 15000000),      # ₱500k - ₱15M
                'COM': (2000000, 50000000),     # ₱2M - ₱50M  
                'IND': (5000000, 200000000),    # ₱5M - ₱200M
                'INF': (10000000, 500000000),   # ₱10M - ₱500M
                'REN': (200000, 5000000),       # ₱200k - ₱5M
                'MIX': (3000000, 100000000),    # ₱3M - ₱100M
            }
            ...
            approved_budget = estimated_cost * random.uniform(0.95, 1.15)  # ±15% variance
            ...
            if status == "CP":
                progress = 100
                actual_completion_date = target_completion + timedelta(
            ...
            project_data = {
                ...
                'location': location,
                'city_province': location.split(',')[-1].strip() if ',' in location else location,
